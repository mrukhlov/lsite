...
jQuery.extend(Mgly.CodeMirrorDiffView.prototype, {
	...
	_markup_changes: function (editor_name1, editor_name2, changes) {
		...
		jQuery('.merge-button').on('click', function(ev){
			$('#merge_to_left').hide();
			$('#merge_to_right').hide();
			var merge_side = this.id;
			if (merge_side.search('lhs') > 0) {$('#upload_diffs').show().text('upload left')} else if (merge_side.search('rhs') > 0){$('#upload_diffs').show().text('upload right')}
			// side of mouseenter
			var side = 'rhs';
			var oside = 'lhs';
